...
def add_punctuation(original_text: str, corrected_text: str) -> str:
    ...
    # If there is only one word in the original text, do not correct the case or add punctuation
    ...
    for i in range(len(corrected_words) - 1):
        if corrected_words[i].endswith('s') and corrected_words[i + 1].istitle():
            corrected_words[i] += "'"
